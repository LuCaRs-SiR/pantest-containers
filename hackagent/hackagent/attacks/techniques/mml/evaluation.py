...
def _build_prompt_prefix(item: Dict[str, Any]) -> str:
    """Build the 'prefix' field from MML item data."""
...
class MMLEvaluation(BaseEvaluationStep):
    ...
    def execute(self, input_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        mml_params = self._raw_config.get("mml_params", {})
        judges_config = self._resolve_judges_from_config(technique_params=mml_params)
        ...
        base_config = self._build_base_eval_config(technique_params=mml_params)
    ...
    def _transform_to_eval_rows(
        ...
        """
        Convert MML items to ``(goal, prefix, completion)`` rows.
...
def execute(
    ...
    """
    step = MMLEvaluation(config=config, logger=logger, client=client)
